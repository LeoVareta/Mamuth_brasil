            ...
            <table className="w-full border-collapse rounded-xl overflow-hidden shadow-md">
              <thead>
                <tr className="text-white" style={{ backgroundColor: '#FF6B0A' }}>
                  ...
                  <th className="py-4 px-1 border-r border-orange-400 font-bold text-[9px] sm:text-xs md:text-sm text-center">{t('tabela.vazaomaxlmin')}</th>
                  <th className="py-4 px-1 border-r border-orange-400 font-bold text-[9px] sm:text-xs md:text-sm text-center">{t('tabela.conexaoencaixe')}</th>
                  <th className="py-4 px-1 border-r border-orange-400 font-bold text-[9px] sm:text-xs md:text-sm text-center">{t('tabela.peso1')}</th>
                  <th className="py-4 px-1 font-bold text-[9px] sm:text-xs md:text-sm text-center leading-tight">{t('tabela.temperaturamaxima')}</th>
                </tr>
              </thead>
              <tbody>
                <tr className="bg-white text-gray-800 border-b border-gray-200">
                  <td className="py-6 px-1 border-r border-gray-200 font-bold text-[10px] sm:text-xs md:text-base text-center">{t('bicos.rotativo600.title')}</td>
              ...
              <div className="rounded-xl shadow-lg border-2 overflow-hidden transition-all duration-300" style={{ borderColor: '#FF6B0A' }}>
                ...
                <div className={`transition-all duration-300 ease-in-out ${aberto === 't1' ? 'h-auto opacity-100 p-5 pt-0' : 'max-h-0 opacity-0'}`}>
                  <div className="grid grid-cols-2 gap-2 text-sm border-t py-4">
                    <p><strong>{t('tabela.modelo')}:</strong> {t('bicos.rotativo600.title')}</p>
                    <p><strong>{t('tabela.pressao')}:</strong> 600</p>
                    <p><strong>{t('tabela.pressao2')}:</strong> 8700</p>
                    <p><strong>{t('tabela.vazaomaxlmin')}:</strong> 80 </p>
                    <p><strong>{t('tabela.conexaoencaixe')}:</strong> 1/4 NPT </p>
                    <p><strong>{t('tabela.peso1')}:</strong> 0,700 </p>
                    <p><strong>{t('tabela.temperaturamaxima')}:</strong> 90°C </p>
                  </div>
                </div>
              </div>
            </div>
